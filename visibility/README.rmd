...
Measurements were taken using a cron job daily from August to November 2015.
    ...
    fs.writeFile("longitudinal_visibility.csv", repoStatsOverTime.table.join("\n"), function(error) {
      if (!error) {
        ...
        shellPromise(" header=`grep year longitudinal_visibility.csv`" +
          " && sed s/$header//g longitudinal_visibility.csv > cleaned.csv" +
          " && echo $header > longitudinal_visibility.csv" +
